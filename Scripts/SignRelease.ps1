<#
  ...
  Usage:
    .\SignRelease.ps1 -Folder "C:\temp" -Repo "trimble-oss/sql-log-shipping-service" -SignTool "C:\Sign\SignFiles.exe" -SignPattern "LogShippingService.exe" -RemoveUnsigned $true
#>
param (
    ...
    [string]$Repo="trimble-oss/sql-log-shipping-service",
    # Path of the Trimble Sign Tool to use for signing the executables.
    [string]$SignTool = "C:\Sign\SignFiles.exe",
    # Pattern to match the executables to sign.  Defaults to "LogShippingService.exe".
    [string]$SignPattern = "LogShippingService.exe",
